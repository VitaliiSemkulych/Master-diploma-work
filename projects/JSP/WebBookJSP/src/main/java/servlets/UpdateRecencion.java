 ...
 * @author Admin
 */
// servlet  process functionality which update recension
@WebServlet(name = "UpdateRecencion", urlPatterns = {"/UpdateRecencion"})
public class UpdateRecencion extends HttpServlet {
   ...
   Connection conn = ConnectionDBUtility.getInstance();
        try (PreparedStatement st = conn.prepareStatement("update recension set recension_text=? where recension_id=?");) {
            ...
            st.setString(1, Formatting.handleEmailText(request.getParameter("modifyRecensionText")));
            st.setLong(2, Long.valueOf(request.getParameter("recensionID")));
            st.executeUpdate();

            request.getSession().setAttribute("modifyRecensionText", null);
            request.getSession().setAttribute("modifyRecensionId", -1);
        } catch (SQLException ex) {
            Logger.getLogger(UpdateRecencion.class.getName()).log(Level.SEVERE, null, ex);
        }
